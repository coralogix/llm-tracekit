...
MICROSOFT_FOUNDRY_SYSTEM = "microsoft_foundry"
...
def extract_foundry_context(kwargs: dict[str, Any]) -> dict[str, Any]:
    """Extract Microsoft Foundry-specific context from extra_body and kwargs."""
    attributes: dict[str, Any] = {}

    extra_body = kwargs.get("extra_body")
    if extra_body and isinstance(extra_body, Mapping):
        agent_ref = extra_body.get("agent_reference")
        if agent_ref and isinstance(agent_ref, Mapping):
            agent_name = agent_ref.get("name")
            if agent_name:
                attributes["gen_ai.microsoft_foundry.agent.name"] = agent_name
            agent_version = agent_ref.get("version")
            if agent_version:
                attributes["gen_ai.microsoft_foundry.agent.version"] = agent_version

    conversation = kwargs.get("conversation")
    if conversation is not None and conversation is not NOT_GIVEN:
        if isinstance(conversation, str):
            attributes["gen_ai.microsoft_foundry.conversation_id"] = conversation
        else:
            conv_id = get_property_value(conversation, "id")
            if conv_id:
                attributes["gen_ai.microsoft_foundry.conversation_id"] = conv_id

    return attributes


def parse_tool_calls(
    tool_calls: list[dict[str, Any] | Any] | None,
) -> list[ToolCall] | None:
...
def choices_to_span_attributes(choices: list, capture_content: bool) -> dict[str, Any]:
    parsed_choices = []
    for choice in choices:
        ...
        parsed_choices.append(
            Choice(
                finish_reason=get_property_value(choice, "finish_reason") or "error",
                role=role,
                content=content,
                tool_calls=tool_calls,
            )
...
def _extract_tools_attributes(tools: list | None) -> dict[str, Any]:
    """Extract tool definitions to span attributes."""
    attributes: dict[str, Any] = {}
...
@attribute_generator
def get_chat_request_attributes(
    kwargs: dict[str, Any], client_instance, capture_content: bool
) -> dict[str, Any]:
    """Build span attributes for chat.completions.create."""
    attributes = {
        **generate_base_attributes(system=MICROSOFT_FOUNDRY_SYSTEM),
        **generate_request_attributes(
    ...
    if seed is not None and seed is not NOT_GIVEN:
        attributes[GenAIAttributes.GEN_AI_OPENAI_REQUEST_SEED] = seed
    ...
    if tools is not None and tools is not NOT_GIVEN:
        attributes.update(_extract_tools_attributes(tools))

    attributes.update(generate_server_address_and_port_attributes(client_instance))
    attributes.update(extract_foundry_context(kwargs))
...
@attribute_generator
def get_chat_response_attributes(result: Any, capture_content: bool) -> dict[str, Any]:
    """Build span attributes from ChatCompletion response."""
    ...
    if usage is not None:
        usage_input_tokens = getattr(usage, "prompt_tokens", None)
        usage_output_tokens = getattr(usage, "completion_tokens", None)
    ...
    if service_tier is not None:
        attributes[GenAIAttributes.GEN_AI_OPENAI_REQUEST_SERVICE_TIER] = service_tier

    return attributes


def _responses_input_content_part_to_text(part: Any) -> str | None:
    """Extract text from a Responses API input content part."""
    if part is None:
        return None
...
def _responses_normalize_input_content(content: Any) -> str | None:
    """Normalize Responses input message content to a single string."""
    if content is None:
        return None
...
def _responses_input_item_to_message(item: Any) -> Message | None:
    """Convert one Responses input list item to a Message."""
    if item is None:
        return None
...
def get_responses_request_attributes(
    ...
    """Build span attributes for responses.create."""
...
def get_responses_response_attributes(
    ...
    """Build span attributes from a Responses Response."""
    usage_input = None
    usage_output = None
    usage = getattr(result, "usage", None)
    if usage is not None:
...
def _embedding_input_to_prompt_messages(embedding_input: Any) -> list[Message]:
...
def get_embedding_request_attributes(
    ...
    """Build span attributes for embeddings.create."""
    attributes: dict[str, Any] = {
...
def get_embedding_response_attributes(
    ...
    """Build span attributes from embeddings response."""
    usage = getattr(result, "usage", None)
    usage_input_tokens = None
    if usage is not None:
