...
@pytest.mark.vcr()
@pytest.mark.asyncio()
async def test_agent_single_turn(span_exporter, instrument):
    simple_agent = Agent(
        name="SimpleAgent",
        model="gpt-4o-mini",
    ...
    assert_choices_in_span(
        span=final_response_span, expected_choices=expected_choices, expect_content=True
    )
...
@pytest.mark.vcr()
@pytest.mark.asyncio()
async def test_agent_tool_usage(span_exporter, instrument):
    @function_tool
    ...
    expected_messages = [
        {"role": "system", "content": "get weather"},
        {"role": "user", "content": prompt},
        {
            "role": "assistant",
            "tool_calls": [
...
@pytest.mark.vcr()
@pytest.mark.asyncio()
async def test_agent_multi_turn(span_exporter, instrument):
    simple_agent = Agent(
        name="SimpleAgent",
        model="gpt-4o-mini",
        instructions="Be a helpful assistant",
...
@pytest.mark.vcr()
@pytest.mark.asyncio()
async def test_agent_tool_execution_error(span_exporter, instrument):
    @function_tool
...
@pytest.mark.vcr()
@pytest.mark.asyncio()
async def test_agent_nested_agent(span_exporter, instrument):
    @function_tool
    async def get_weather(city: str) -> str:
        return f"The weather in {city} is currently 25°C and clear."
    ...
    assert (
        len(spans) == 9
    )  # response -> response -> function -> response -> agent data ->
