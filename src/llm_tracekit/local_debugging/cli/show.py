...
from llm_tracekit.local_debugging.cli.list import _format_timestamp, _parse_conversation_from_span
...
def show_span(traces_directory: Optional[str], span_id):
    ...
    if len(expected_span) != 1:
        console.print(f"[red]Session number {span_id} not found. Run 'list' command to see available sessions.[/red]")
        return
    
    span = expected_span[0]
    
    # Show general session id identifier
    console.print(Panel(f"[bold]Session ID:[/bold] {span['span_id']}", 
